...
const aiCategorisFetch = () => {
  ...
  fetch(url)
  .then(res => res.json())
  .then(data => showAiCategoris(data.data.tools))
}
...
data.slice(0 ,6).forEach(singleAiCategories => {
  ...
  showAiCategoris(data.sort(shorting))
   })
}
// show categories end
  ...
  <div class="col">
    <div class="card p-3">
      ...
      <div class="card-body text-center " >
        ...
        <p class="card-text">${allData.input_output_examples[0].output.slice(0,100) ? allData.input_output_examples[0].output.slice(0,100): 'No! Not Yet! Take a break!!!'}</p>
      </div>
    </div>
  </div>
</div>
...
// see all button data fetch start 
 const aiCategorisBtn = () => {
  ...
  fetch(url)
  .then(res => res.json())
  .then(data => showAiCategoris2(data.data.tools))
}
// see all button data fetch end

// see all button data show start
const showAiCategoris2 = data2 => {
...
document.getElementById('btn-Sortby').addEventListener('click', function(){
  const aiContainer = document.getElementById('ai-container')
  aiContainer.innerHTML ='';
  showAiCategoris2(data2.sort(shorting))
 })
}
